...
function updateClonedElementIds(clonedElement) {
    const elementType = clonedElement.data('type');
    const newId = elementType + '-field-' + fieldCounter++;
    clonedElement.attr('id', newId);

    if (elementType === 'radio') {
        const listGroupId = 'list-group-' + listGroupCounter++;
        clonedElement.find('.list-group').attr('id', listGroupId);

        // Update radio button names and IDs for the cloned radio group
        clonedElement.find('.option-radio').each(function(index) {
            $(this).attr('name', newId + '-option').attr('id', newId + '-option-' + index);
            $(this).next('label').attr('for', newId + '-option-' + index);
        });
    }
}
